...
Sourced from CI verification while preparing a public post (see conversation log, 2026-08-25) — Roadmap v8's coverage of the external assessment surfaced no findings about lock-acquisition atomicity, but running the new 3.10/3.11/3.12 CI matrix (AUTO-069) for the first time immediately caught a real, reproducible race in `acquire_lock` on the 3.11 leg.

### AUTO-070 — Fix a real race window in acquire_lock's atomicity guarantee
Priority: P0
Status: DONE

Goal: `acquire_lock` (`src/autonomous_forge/lock.py:94-132`) creates the lock file via `os.open(path, O_CREAT|O_EXCL|O_WRONLY)` — atomic — then writes the JSON payload to it as a separate step afterward. A second racer that hits `FileExistsError` in that window reads the still-empty file; `_read_lock` fails to parse it and returns `(None, "unknown")`; the code treats "unparseable" the same as "no valid lock, safe to delete" and unlinks the first racer's not-yet-written lock file, then successfully recreates its own. The first racer, unaware, then writes its payload into a detached, unlinked file descriptor and also reports success. `tests/test_lock.py::TestAcquireLock::test_concurrent_acquire_never_succeeds_twice` caught this directly on the Python 3.11 CI leg (`assert 2 == 1`, both results `("ok", ForgeLock(...))` for the identical path) — not a flake, a real bug, first observed because this was the first time the new 3.10/3.11/3.12 CI matrix (AUTO-069) actually ran.
Why it matters: This directly contradicts the module's own documented guarantee ("two processes racing to acquire at the same instant can never both succeed") and this project's own repeated Roadmap v8 theme — fail-open gaps in exactly the kind of safety claim the tool exists to make credible. The target audience (people running multiple coding agents against the same repo) is exactly who would hit and notice this.
Scope: Eliminate the race window by writing the fully-formed payload to a private temp file first (`tempfile.mkstemp` in the same directory, guaranteed unique/atomic), then atomically publishing it via `os.link` (fails with `FileExistsError` if the destination already exists — same atomicity guarantee as `O_CREAT|O_EXCL`, but by the time the lock path has any content, that content is always complete). No window where a partially-written file is visible at the final path. Update the module docstring's atomicity claim to describe the new mechanism accurately.
Expected files or areas: src/autonomous_forge/lock.py, tests/test_lock.py, docs/COMMANDS.md (if it documents the locking guarantee)
Acceptance criteria: `test_concurrent_acquire_never_succeeds_twice` passes reliably; a stress-test loop (e.g. running the concurrent-acquire test N times, or with more than 2 threads) shows no double-success. CI green on all three Python versions.
Validation: `python -m pytest` — 476 tests pass (unchanged count; fix + doc updates only, no new tests needed since the existing barrier-synchronized two-thread concurrency test already provides direct regression coverage and now passes reliably). `ruff check .`/`mypy` — clean. Stress-tested well beyond the committed test: a standalone script ran 200 trials of 4 racing threads (vs. the committed test's 2) directly against `acquire_lock`, zero double-successes. Manually verified no orphaned temp files remain after acquire+release, and after a blocked (LockHeldError) attempt. `forge lint-plan` — ok.
...
Validation: `ruff check .` — clean, now includes `S310`. `ruff check src --select S` — clean, all 18 original findings resolved (2 `S310` fixed for real, 16 `S603`/`S607` noqa'd with reasons). `python -m pytest --cov=autonomous_forge` — 477 tests pass (476 baseline + 1 new), 88.75% coverage, above AUTO-071's 80% floor. `mypy` — clean. Manually verified the `S310` fix is a real runtime guarantee, not just a static-analysis suppression: constructed a `ForgejoClient` directly with an `http://` base URL (bypassing `resolve_base_url`'s own check) and confirmed `_request` still refuses with a clean `RuntimeError`, not a raw connection attempt.
Risks or assumptions: Blanket-noqa'ing `S603`/`S607` risks masking a genuinely unsafe new subprocess call added later at one of the 16 existing sites, if a future edit changes what's passed to it — each noqa comment states precisely why the *current* call is safe (fixed argv list, no shell interpolation) except `validate.py`'s, which is honestly different: it's the one place that's *meant* to run arbitrary code by design (the validation command), noqa'd with a pointer to SECURITY.md's "not a sandbox" section rather than a fixed-argv claim that wouldn't be true there. A truly new subprocess call site elsewhere in the codebase still surfaces via the advisory step, since noqa only silences the specific annotated line.
Notes: Narrows "adopt Ruff security rules more broadly" (deferred in AUTO-055, made advisory-only in AUTO-068) into a right-sized enforcement decision instead of an all-or-nothing one. The `S310` fix added a real runtime `https://` check in `ForgejoClient._request` itself (not just relying on `resolve_base_url`'s caller-side validation from AUTO-067) — defense in depth regardless of how the client is constructed, with a regression test in `tests/test_forgejo_client.py`.

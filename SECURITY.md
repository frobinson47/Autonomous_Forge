...
- **Forgejo sync makes real API calls** to whatever instance is configured (`--base-url`, `FORGEJO_BASE_URL` env var, or `.forge/config.toml`'s `forgejo_base_url` — must be `https://`; AUTO-067), using a token you provide. `forge sync --report-orphans`/`--import-orphans` are read-only against Forgejo; the default mode creates and updates issues, labels, and milestones. Transport failures (DNS, connection, timeout, malformed response) are caught and reported as a clean error, not a stack trace — but this is still real code making real network calls with your token; only point it at a Forgejo instance and token you trust.
